...
// @ts-ignore
import { history, ApplyPluginsType } from 'umi';
...
import { message, notification } from 'PluginUmiRequestCompat';
...
const getRequestMethod = () => {
  ...
  const requestConfig: RequestConfig = getPluginManager().applyPlugins({
    key: 'request',
...
`;
  ...
  api.onGenerateFiles(() => {
    const { dataField = 'data' } = api.config.umiRequestCompat;
    try {
      let formatResultStr;
      if (dataField === '') {
      ...
      api.writeTmpFile({
        ...
        content: requestTemplate
          ...
          .replace('PluginUmiRequestCompat',api.config.antd ? 'antd' : './noop')
      });
...
`,
      ...
      api.writeTmpFile({
        ...
        content: `
export {
  request, useRequest, UseRequestProvider
} from './request';
`,
      });

      // TODO: runtimeConfig 用 RUNTIME_TYPE_FILE_NAME
      // https://github.com/umijs/umi/pull/8802
      api.writeTmpFile({
        path: "runtimeConfig.d.ts",
        content: `
